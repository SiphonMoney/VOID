          ...
          const responseHandler = (event) => {
            ...
            if (event.data.type === 'ANONYMAUS_SOLANA_TO_PAGE' && event.data.payload.requestId === requestId) {
              ...
              if (error) {
                ...
                if (result && result.signature && transactionData?.serialized) {
                  window.__ANONYMAUS_TEE_SIG_MAP__[transactionData.serialized] = result.signature;
                  window.__ANONYMAUS_LAST_TEE_SIG__ = result.signature;
                  window.__ANONYMAUS_LAST_TEE_SIG_TS__ = Date.now();
                  ensureSendRawPatched();
                }
                if (result && result.signature && transaction) {
                  const sigBytes = base58ToBytes(result.signature);
                  if (sigBytes) {
                    try {
                      ...
                      } else if (transaction.signatures?.length) {
